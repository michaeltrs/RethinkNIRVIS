...
def main(local_rank):
    ...
    #     if ep > 0:
    #         backbone.eval()
    #     for step, (img, labels) in enumerate(get_dataloader(cfg.nirvis_root_dir, cfg.nirvis_paths_file, 50, mode='pretrain',
    #                                                         rank=local_rank, world_size=cfg.world_size,
    #                                                         num_workers=0)):  # train_loader):
    #         # print("step %d of %d" % (step, len(train_loader)), img.shape)
    #
    #         labels = labels.reshape(-1).cpu()
    #         # with torch.no_grad():
    #         features = backbone(img.to(local_rank), return_feats=False)
    ...
    print((torch.eye(W.shape[0]).to(W.device) - torch.matmul(W, W.T)).mean())
    ...
    # for step, (img, labels) in enumerate(get_dataloader(cfg.nirvis_root_dir, cfg.nirvis_paths_file, 100, mode='pretrain',
    #                    rank=local_rank, world_size=cfg.world_size, num_workers=0)):  # train_loader):
    #     _ = backbone(img, return_feats=False)
    # backbone.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='iresnet18_init-WF600k_CASIA1of10_RCT', help='config file name')
