...
class IResNet(nn.Module):
    ...
    def forward(self, x, T=0, return_feats=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.prelu(x)
...
def iresnet9(pretrained=False, progress=True, **kwargs):
    return _iresnet('iresnet9', IBasicBlock, [1, 1, 1, 1], pretrained,
                    progress, **kwargs)
