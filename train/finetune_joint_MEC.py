...
def main(local_rank):
    ...
    for step, (img, labels) in enumerate(get_dataloader(cfg.nirvis_root_dir, cfg.nirvis_paths_file, cfg.nirvis_batch_size, mode='pretrain',
                       rank=local_rank, world_size=cfg.world_size, num_workers=0)):
    ...
    torch.distributed.all_reduce(weights)
    torch.distributed.all_reduce(label_counts)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for step, (img, label) in enumerate(train_loader):
            ...
            loss = lambda_vis * rgb_cls_loss + nirvis_cls_loss
